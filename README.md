...
go-grpc-middleware defines its own `Level` enum. `slogcp-grpc-adapter` maps these into `slog.Level` using a small default mapping:

- `LevelDebug` → `slog.LevelDebug`.
- `LevelInfo` → `slog.LevelInfo`.
- `LevelWarn` → `slog.LevelWarn`.
- `LevelError` and unknown values → `slog.LevelError`.

This keeps the interceptor’s idea of “debug/info/warn/error” aligned with both `slog` and `slogcp`’s Cloud Logging severities. If you need different semantics, you can supply your own mapping function (see **Customization** below).
...
When you need finer control over how go-grpc-middleware’s logging levels map onto slog (and thus Cloud Logging severities), provide a custom mapper:
...
The adapter will always fall back to its default mapping when the mapper is `nil`, so it is safe to inject helper options that sometimes choose not to override the mapping.
