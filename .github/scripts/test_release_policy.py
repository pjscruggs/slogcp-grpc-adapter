...
class ReleasePushRangeTests(unittest.TestCase):
    ...
    def test_baseline_must_declare_a_version_even_without_module(self):
        for has_module in (False, True):
            with self.subTest(has_module=has_module):
                if has_module:
                    self.write("go.mod", "module example.invalid/module\n")
                before = self.commit()
                self.library("v1.0.1")
